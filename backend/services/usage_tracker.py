...
class UsageTracker:
    ...
    def log(self, service: str, model: str, input_tokens: int, output_tokens: int):
        ...
        print(
            f"[API] {service} | 토큰 {input_tokens:,}+{output_tokens:,} | "
            f"이번 ${cost:.4f} (≈{cost_krw:.0f}원) | 누적 ${cumulative:.4f} (≈{cum_krw:.0f}원)"
        )
    ...
    def summary(self) -> dict:
        ...
        for r in records:
            s = by_service.setdefault(r.service, {"calls": 0, "input_tokens": 0, "output_tokens": 0, "cost_usd": 0.0})
            s["calls"] += 1
