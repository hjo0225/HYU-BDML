...
async def llm_engagement(
    ...
    *,
    ...
    roster = "\n".join(f"- {a['agent_id']}: {a['name']} — {a.get('summary', '')[:80]}" for a in agents_meta)
    user = (
        f"현재 소주제: {subtopic}\n직전 발언: \"{last_utterance or '(없음)'}\"\n참여자:\n{roster}\n\n"
        "각 참여자가 ① 지금 발언하고 싶어할 관심도(interest) ② 직전 발언에 동의·반박으로 반응할 가능성"
        "(reaction)을 각각 0~1 로 추정하세요. JSON 만 출력: "
        '{"<agent_id>": {"interest": 0.0, "reaction": 0.0}, ...}'
    )
    try:
        raw = (await chat_completion(system=_ENG_SYSTEM, user=user, model=config.CHAT_MODEL,
                                     temperature=0.3, max_tokens=400, mock=mock)).strip()
        if raw.startswith("```"):
            raw = raw.split("\n", 1)[1].rsplit("```", 1)[0]
        data = json.loads(raw)
        out: dict[str, dict[str, float]] = {}
        for a in agents_meta:
            d = data.get(a["agent_id"]) or {}
            out[a["agent_id"]] = {
