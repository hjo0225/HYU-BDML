...
Accepted (2026-04-30) · Amended (2026-05-03): L3(엄격 검증 버튼) 제거 — 사용자 트리거식 라이브 judge 호출은 비용·UX 모두 부담이라 라이브에서 내림. judge 인프라는 오프라인 평가(`eval_lab_faithfulness`)에서 그대로 사용.
...
| 층위 | 내용 | 비용 | 라이브 |
|---|---|---|---|
| **L1. 트윈 카드 점수** | 카테고리별 충실도 막대 + 종합 % (오프라인 `eval_lab_faithfulness` 결과) | 사전 계산 | 정적 |
| **L2. 메시지 인용** | A(LLM 자가 인용) + B(임베딩 검증) 하이브리드. 🟢/🟡/🟠/⚪ 신뢰도 배지 + 펼침 패널 | 답변 임베딩 1회/턴 | 자동 |
| ~~**L3. 엄격 검증 버튼**~~ | ~~메시지 hover 시 "🧪 엄격 검증" → judge 호출 → verdict 카드~~ | ~~judge 1회/메시지~~ | **2026-05-03 제거** |
...
- ~~L3 judge: gpt-4o 기준 메시지당 ~$0.005~$0.01.~~ 라이브 L3 제거됨 (2026-05-03).
- 평가 일괄 실행: 50명 × ~20 카테고리 × 채팅 + judge ≈ $5~$10 일회성. (오프라인만)
...
**−** 라이브 엄격 검증을 내려서 사용자가 단일 메시지의 일관성을 즉시 검증할 수단이 없음 — 카테고리 단위 사전 점수(L1)로만 가늠.
